...
- [x] 核对最终分支、提交、完整范围与无关文件，确认所有本地门槛通过；准备PR说明，包含问题/结果、范围、风险分类、检查命令与结果、行为/宿主证据、API/数据影响及残余风险。
...
- [x] 结构/安装/维护工具及其有效行为测试。
- [x] 40基础场景、规定重复、至少10个留出及必需原生证据（AT-16浏览器变体按环境缺口如实保留）。
- [x] 任务完整性、权限、验证与加载成本的诚实对照报告。
- [x] 双语说明、2.0.0版本材料、迁移/恢复及支持范围说明。
- [x] 完整范围审查及可审PR材料；远端CI在T34取得（5/5绿）。

公开发布与本机实际安装只有在相应T36/T37完成后才能报告完成。全部任务和清单已勾选并完成；发布与本机安装的实际结果见各任务实施记录与t37-global-switch/switch-record.json。
