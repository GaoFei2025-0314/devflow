...
4. If a read is partial or truncated, identify the returned coverage and fetch the missing section only when the pending decision depends on it. Large examples may remain unread when their boundary or pattern is irrelevant; applicable constraints may not.
