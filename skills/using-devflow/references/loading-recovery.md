...
2. Select the canonical skill source and read its entrypoint. Follow only references needed for the current decision or action, including any canonical contract that the entrypoint makes applicable.
3. Reuse required sections already available when their source identity, task phase, constraints, and relevant rules are unchanged. Load a newly relevant domain when the work enters it.
4. If a read is partial or truncated, identify the returned coverage. When one orchestration call combines nested reads, use bounded groups or ranges appropriate to the interface and assess coverage in the aggregate outer return: nested command success, complete native stdout, or a per-command output limit does not establish that the outer call returned all selected content. Recover missing applicable content before a dependent decision, while reusing coverage that remains valid. Large examples may remain unread when their boundary or pattern is irrelevant; applicable constraints may not.
