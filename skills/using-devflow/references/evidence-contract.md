...
For review and feedback-processing results, identify both sides of any comparison: the reviewed target and the applicable comparison baseline, preserving which supplied revision or state identifier belongs to each side; for a non-Git comparison, identify each side by its applicable artifact or state description. Carry item dispositions that distinguish supported findings from optional suggestions; distinguish findings supplied by a human or agent from review actually performed for the current result; and state whether that performed review was self-review or independent, including any material limit. Preserve supporting verification and unresolved dependencies. These facts may be stated compactly or supplied through a stable cited review record; they do not require another mandatory report, an invented commit baseline, an unrelated Git operation, a fresh check ritual, an extra review tier, or new authorization. When either comparison side, its identifier, or the review origin cannot be verified, state that limit instead of inventing it.
