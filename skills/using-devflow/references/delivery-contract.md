...
| **Push / pull request** | The package satisfies the project's PR timing policy; review material identifies the exact branch, target, scope, risk, evidence, and known limits. Required local gates must pass before presenting the package as ready. | Push and PR creation need applicable authorization. An implementation grant or a complete package does not synthesize it. |
...
- A required failed check blocks its dependent completion or action gate. Keep the failure and any later retry in the record; do not replace history with the final green result. Know which gate each failed check actually blocks: a failed required check blocks completion claims and the merge or release gate it protects, while preparation that the project authorizes — such as opening a reviewable PR that discloses the red check honestly — remains a legal preparation step unless the project's policy explicitly gates PR creation itself on that check.
...
- Changes involving logic, state, permissions, dependencies, other listed risk surfaces, or uncertain impact may be pushed and opened as a reviewable PR when those actions are authorized, but merge waits for explicit approval of that concrete PR. Passing CI does not supply that approval.
