...
- **Behavior evaluation**: 40 acceptance scenarios (65 variants) with blind packets, independent judgment, paired V1.3.1/V2 regression, 10 holdout scenarios, and an honest repair loop. Final aggregate: candidate 497 pass / 10 fail / 2 unknown against baseline 501 / 8 / 0; release verification still exits 1 because the evidence contract keeps legacy failures and AT-16 is blocked on both sides by a host with no browser. Median loaded bytes per episode rose from 16,647 (V1.3.1) to 51,613 — an exploratory metric that offsets nothing. Raw traces stay in a local evidence workspace outside this repository; the summary is in `docs/devflow/v2-evaluation-summary.md`.
